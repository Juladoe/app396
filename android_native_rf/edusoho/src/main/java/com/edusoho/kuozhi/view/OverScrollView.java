...
public class OverScrollView extends FrameLayout implements OnTouchListener
{
	...
	static final float OVERSHOOT_TENSION = 0.55f;
	...
	/** header height db  --suju */
	private int refreshHeadHeight = 0;
	
	public void setRefreshHeadHeight(int height)
	{
		refreshHeadHeight = height;
	}
	...
	private void initBounce(AttributeSet attrs)
	{
		if (attrs != null) {
			TypedArray ta = mContext.obtainStyledAttributes(attrs, R.styleable.OverScrollView);
			refreshHeadHeight = (int) ta.getDimension(R.styleable.OverScrollView_refreshHeight, 0);
		}
	...
	private int defChilePadding = 1500;
	
	public void initChildPointer()
	{
		child = getChildAt(0);
		child.setPadding(0, defChilePadding, 0, defChilePadding);
	...
	private void initScrollView()
	{
		...
		post(new Runnable()
		{
			public void run()
			{
				if (getChildCount() > 0) {
					// refreshHeadHeight
					scrollTo(0, child.getPaddingTop() + refreshHeadHeight);
				}
			}
		});
	}
	...
	/**
	 * by suju child fast move to top
	 */
	public void initChildToTop()
	{
		scrollTo(0, child.getPaddingTop() + refreshHeadHeight);
	}
			...
			if (topOfVisibleView < paddingTop || topOfVisibleView > contentBottom - displayHeight)
			{
				if (topOfVisibleView < paddingTop)
				{
					mScroller.startScroll(
							0, topOfVisibleView, 0, paddingTop - topOfVisibleView + refreshHeadHeight, 1000);
	...
	public void scrollTo(int x, int y)
	{
		...
		if (getChildCount() > 0)
		{
			...
			int delta = defChilePadding - y;
			if (mMoveListener != null) {
				mMoveListener.invoke(delta);
			}
		}
	}
	...
	private boolean overScrollView()
	{
		...
		if (currScrollY < contentTop)
		{
			...
			scrollBy = contentTop - currScrollY + refreshHeadHeight;
		...
		} else if (currScrollY > contentTop 
				&& (currScrollY - contentTop) < refreshHeadHeight) {
			onOverScroll(currScrollY);
			scrollBy = refreshHeadHeight - (currScrollY- contentTop);
		}

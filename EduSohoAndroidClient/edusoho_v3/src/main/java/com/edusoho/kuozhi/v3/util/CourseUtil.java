...
public class CourseUtil {
    ...
    public static void addCourse(final CourseParamsBuilder builder, final OnAddCourseListener
        ...
        String courseId = builder.params.get("targetId");
        String totalPrice = builder.params.get("totalPrice");
        RequestUrl url;
        if(Double.parseDouble(totalPrice)>0){
            url = EdusohoApp.app
                    .bindNewUrl(String.format(Const.CREATE_ORDER_CONSIGINEE, courseId),true);
                ...
                public void onResponse(String response) {
                    ...
                    if (response != null) {
                        try {
                            ...
                            boolean requireConsigneeInfo = jsonObject.getBoolean("requireConsigneeInfo");
                            if (requireConsigneeInfo) {
                                isRequireConsigneeInfo(builder, "bookpay");
                            } else {
                                isRequireConsigneeInfo(builder, "coursepay");
                            }
            ...
            url = EdusohoApp.app.bindUrl(Const.CREATE_ORDER,true);
    ...
    public static void isRequireConsigneeInfo(CourseParamsBuilder builder, String s){
        final String url = String.format(
                Const.MOBILE_APP_URL,
                EdusohoApp.app.schoolHost,
                String.format("main#/"+s+"/%s/%s",
                        builder.targetId
                        , "course")
        );

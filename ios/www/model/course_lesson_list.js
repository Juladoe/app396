...
*/
    ...
    exports.init_course_lesson_data = function(course_id, lesson_id) {
        ...
        simpleJsonP(schoolHost + "/courses/" + course_id + '/items?callback=?&token=' + token,
        function(data) {
            if (data) {
                ...
                for (var name in data) {
                    ...
                    list_str += "<li id='lesson_" + lessonItem.id + "'" + " type='" + lessonItem.type + "'" + " title='" + title + "'" + " mediaUri='" + lessonItem.mediaUri + "'" + " onclick='course_lesson_list_model.selCourseLessonMenu(" + lessonItem.id + ");'><a class='" + sel_class + "'>" + "<i class='lesson_type_normal_color fa " + item_type + "'></i>&nbsp;" + title + "</a><textarea class='tab_hide content'>" + lessonItem.content + "</textarea></li>";
                }
    ...
    function loadLesson(courseId, lessonId) {
        var token = appstore_model.getToken();
        simpleJsonP(schoolHost + "/courses/" + courseId + "/lessons/" + lessonId + "?callback=?&token=" + token,
        function(lessonItem) {
            ...
            setLessonContent(lessonContent, lessonItem.type, lessonId, lessonItem.mediaUri, lessonItem.mediaSource);
        });
    }
    ...
    exports.finish_lesson = function() {
        var token = appstore_model.getToken();
        simpleJsonP(schoolHost + "/courses/" + course_lesson_list_model.courseId + "/lessons/" + course_lesson_list_model.lessonId + "/learn?callback=?&token=" + token,
        function(data) {
            if (data.error) {
    ...
    exports.cancel_lesson = function() {
        var token = appstore_model.getToken();
        simpleJsonP(schoolHost + "/courses/" + course_lesson_list_model.courseId + "/lessons/" + course_lesson_list_model.lessonId + "/unlearn?callback=?&token=" + token,
        function(data) {
            if (data.error) {
                $("#afui").popup(data.message);
                return;
    ...
    exports.learn_status = function(lessonId) {
        if (appstore_model.checkIsLogin()) {
            ...
            var token = appstore_model.getToken();
            simpleJsonP(schoolHost + "/courses/" + course_lesson_list_model.courseId + "/lessons/" + lessonId + "/learn_status" + '?callback=?&token=' + token,
            function(data) {
                if (data.error) {
                    $("#afui").popup(data.message);
                    return;

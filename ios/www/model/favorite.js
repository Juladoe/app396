...
<div title="收藏的课程" id="favorite" class="panel" data-footer='none' data-header="normal_header">
					...
					<a class="card-bg-a" onclick="load_courseinfo_page('${id}', 1);">
						<table style="width:100%;" border="0" cellpadding="0" cellspacing="0">	
							<tr class="card-bg-line" valign="top">
								<td style="width:48%;">
									<img class="favorite_course_pic" src="${cb:middlePicture}" width="100%" height="90" />
								</td>
    ...
    exports.unFavorite = function(course_id, event) {
        ...
        simpleJsonP(schoolHost + "/courses/" + course_id + "/unfavorite" + '?callback=?&token=' + token,
        function(data) {
            if (data.error) {
    ...
    exports.favorite = function(course_id) {
        var token = appstore_model.getToken();
        simpleJsonP(schoolHost + "/courses/" + course_id + "/favorite" + '?callback=?&token=' + token,
        function(data) {
            if (data.error) {
                showToast(data.message);
                return;
    ...
    exports.load_data = function(isappend) {
        ...
        var token = appstore_model.getToken();
        simpleJsonP(schoolHost + "/me/favorite_courses" + '?callback=?&token=' + token,
        function(data) {
            if (data.error) {
            ...
            list_str = zy_tmpl($("#favorite_list_item").val(), data.data, zy_tmpl_count(data.data),
            function(a, b) {
                switch (b[1]) {
                case "middlePicture":
            ...
            $(".favorite_course_pic").each(function() {

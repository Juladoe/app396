...
(function($) {
        ...
        */
        ...
        updateHeaderElements: function(elems, goBack) {
            ...
            if (this.prevHeader) {
                ...
                var from = goBack ? "1px" : "-1px";
                var to = goBack ? "-1px" : "1px";
        ...
        loadContentData: function(what, newTab, back, transition) {
            ...
            if (back) {
                if (this.history.length > 0) {
                    ...
                    */
                    if (el) this.setBackButtonText("返回");
            ...
            */
            else if (this.activeDiv.getAttribute("data-title")) this.setBackButtonText("返回");
            else this.setBackButtonText("返回");
            if (what.getAttribute("data-title")) {
                this.setTitle(what.getAttribute("data-title"));
            }
            if (newTab) {
                this.setBackButtonText("返回");
                /*
                原始方式
            ...
            var hasHeight = what.getAttribute("data-height");
            if (hasHeight) {
                $("#header").css('height', hasHeight + "px");
            } else {
        ...
        launch: function() {
            ...
            $(this.header).html(" <a id='backButton' class='fa-angle-left'></a><h1 id='pageTitle'></h1>" + this.header.innerHTML);
    ...
    $.maskblockUI = function (opacity) {
        ...
        $.query("BODY DIV#loadmask").bind("touchmove", function (e) {
            e.preventDefault();
        });
        uiMaskBlocked = true;
    };

    $.unMaskblockUI = function () {
        uiMaskBlocked = false;
        $.query("BODY DIV#loadmask").unbind("touchstart");
        $.query("BODY DIV#loadmask").unbind("touchmove");
        $("BODY DIV#loadmask").remove();
    };
